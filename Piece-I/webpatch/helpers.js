...
// ====================
function blinkBang(bangId) {
  var bang = document.getElementById(bangId);
  var bangCircle = bang.getElementById("BangCircle");
  var color = window.pd4webGuiValues[bangId];
  if (color === undefined) {
    color = "black";
  }
  bangCircle.style.fill = color;
  setTimeout(function () {
    bangCircle.style.fill = "#fafafa";
  }, 200);
}

// ====================
function mapNumberToColor(number) {
  var hue = (number / 3000) * 360;
...
function JS_setFloat(symbol, value) {
  if (symbol.includes("ui_bang")) {
    symbol = symbol.replace("ui_", "");
    blinkBang(symbol);
    return;
  }
  if (symbol.includes("ui_color")) {
    var number = symbol.replace("ui_color", "");
    number = parseFloat(number);
    var color = mapNumberToColor(value);
    var varName = "bang" + number;
    window.pd4webGuiValues[varName] = color;
    return;
  }
  if (symbol.includes("ui_totalDuration")) {
    var totalDurationDiv = document.getElementById("duracaoTotal");
    var number = symbol.replace("ui_totalDuration", "");
    totalDurationDiv.style.width = value + "%";
    return;
  }
